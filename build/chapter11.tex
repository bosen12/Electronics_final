...
\note{輸出端 A 不接負載；以主要直流路徑估算}
\textbf{$V_i=V_L=0.2$ V：}$Q_4$ 飽和把 $Q_3$ 基極拉低 $\Rightarrow Q_3$ 截止。主要電流經 $R_3\to Q_4$ 之 B--E $\to$ 輸入端：
\[ I_L=\frac{V_{CC}-V_{BE(on)}-V_L}{R_3}=\frac{5-0.7-0.2}{4\text{k}}=1.025\ \text{mA},\quad P_L=V_{CC}I_L\approx5.13\ \text{mW}. \]
\textbf{$V_i=V_H=3.6$ V：}$Q_4$ 進入反相主動模式，電流經 $R_3\to Q_4$ 之 B--C $\to Q_3$ 基極導通，
$Q_4$ 基極箝在 $V_{BC}+V_{BE,Q3}=1.4$ V；同時 $R_2$ 經導通的 $Q_3$ 對地有電流：
\[ I_H\approx\frac{V_{CC}-1.4}{R_3}+\frac{V_{CC}-V_{CE,Q3}}{R_2}
   =\frac{3.6}{4\text{k}}+\frac{5-0.2}{1.6\text{k}}\approx0.9+3.0=3.9\ \text{mA},\ \ P_H\approx19.5\ \text{mW}. \]
\[ P=\tfrac12(P_L+P_H)\approx\tfrac12(5.13+19.5)\approx12.3\ \text{mW}. \]
\ans{$P_{avg}\approx12.3$ mW（依上述路徑估算；確切值依電路節點接法略有出入）。}
...
\sol 輸出高電位由圖騰柱上臂 $Q_2$（射極隨耦）串二極體 $D$ 提供：
\[ V_o=V_{CC}-V_{CE(sat),Q2}-V_{D}=5-0.2-0.7=4.1\ \text{V}. \]
\textbf{(1)} 流入負載的輸出電流
\[ I_o=\frac{V_o}{R_L}=\frac{4.1}{1\text{k}}=4.1\ \text{mA}. \]
\textbf{(2)} 此電流由 $V_{CC}$ 經 $R_4(130\,\Omega)$、$Q_2$、$D$ 供給，故電源消耗
\[ P\approx V_{CC}\,I_o=5\times4.1\text{m}=20.5\ \text{mW}. \]
\ans{(1) $I_o\approx4.1$ mA。\;(2) $P\approx20.5$ mW。}

%---------------------------------------------------------------
\prob{承上題，但 $R_L=10$ k$\Omega$。}
\sol 輸出高電位仍 $V_o\approx4.1$ V：
\[ I_o=\frac{4.1}{10\text{k}}=0.41\ \text{mA},\qquad P\approx V_{CC}I_o=5\times0.41\text{m}=2.05\ \text{mW}. \]
\ans{$I_o\approx0.41$ mA，$P\approx2.05$ mW。（負載越輕，輸出電流與耗電越小。）}
...
\sol $Q_1$ 為射極隨耦器，基極電流經 $R_{ON}$ 由 $V_{DD}$ 供給，而對電容充電的射極電流 $=(\beta+1)\times$ 基極電流。故電容所見之等效電阻
\[ R_{eq}=\frac{R_{ON}}{\beta+1}=\frac{100}{101}\approx0.99\ \Omega,\qquad
   \tau=R_{eq}C=\frac{R_{ON}C}{\beta+1}=\frac{100\times10\text{p}}{101}\approx9.9\ \text{ps}. \]
\note{以一階 $RC$ 充電近似，$Q_1$ 提供 $(\beta+1)$ 倍電流}
$V_o$ 由 $V_L$ 充向 $V_{DD}-V_{BE}$，充至 $V_H$（約終值）所需時間約為數個 $\tau$：
\[ t\approx 3\tau\approx 3\times9.9\text{ps}\approx30\ \text{ps}. \]
\ans{等效時間常數 $\tau=R_{ON}C/(\beta+1)\approx9.9$ ps，充至 $V_H$ 約 $\sim30$ ps——比純 $R_{ON}C=1$ ns 快約 $(\beta+1)$ 倍，正是 BiCMOS 速度優勢所在。}
...
\sol 與題 9 對稱：下拉 npn $Q_2$ 之基極電流經 $R_{ON}$ 提供，放電（集極）電流為其 $(\beta+1)$ 倍，等效時間常數
\[ \tau=\frac{R_{ON}C}{\beta+1}=\frac{100\times10\text{p}}{101}\approx9.9\ \text{ps},\qquad
   t\approx3\tau\approx30\ \text{ps}. \]
\ans{$\tau\approx9.9$ ps，放電至 $V_L$ 約 $\sim30$ ps（同樣被 BJT 電流增益加速約 $(\beta+1)$ 倍）。}
