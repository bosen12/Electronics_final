...
\end{itemize}
電晶體電流（本書定義 $k=\mu C_{ox}\frac{W}{L}$）：
\begin{formulabox}
\[ \text{飽和區：}\;I_D=\tfrac{1}{2}k(V_{GS}-V_t)^2,\qquad
   \text{三極管區：}\;I_D=k\!\left[(V_{GS}-V_t)V_{DS}-\tfrac{1}{2}V_{DS}^2\right] \]
\end{formulabox}
...
$I_D=\dfrac{V_{DD}-V_o}{R}$。代入三極管電流 $I_D=k\!\left[(V_{GS}-V_t)V_o-\tfrac12 V_o^2\right]$，$V_{GS}-V_t=3$ V。

\textbf{(1) $R=10$ k$\Omega$：}（電流以 mA、電壓以 V）
\[ 1\!\cdot\!\big(3V_o-0.5V_o^2\big)=\frac{5-V_o}{10}\ \Rightarrow\ V_o^2-6.2V_o+1=0
   \ \Rightarrow\ V_o=0.166\ \text{V}. \]
（取小根；$V_o<V_{GS}-V_t=3$ V 確在三極管區。）
\[ I_D=\frac{5-0.166}{10}=0.483\ \text{mA},\qquad
   P=V_{DD}I_D=5\times0.483=2.42\ \text{mW}. \]

\textbf{(2) $R=1$ k$\Omega$：}
\[ 1\!\cdot\!\big(3V_o-0.5V_o^2\big)=\frac{5-V_o}{1}\ \Rightarrow\ V_o^2-8V_o+10=0
   \ \Rightarrow\ V_o=1.55\ \text{V}. \]
\[ I_D=\frac{5-1.55}{1}=3.45\ \text{mA},\qquad P=5\times3.45=17.25\ \text{mW}. \]
\ans{(1) $V_o\approx0.17$ V，$P\approx2.42$ mW。\quad (2) $V_o\approx1.55$ V，$P\approx17.25$ mW。\;（$R$ 小則 $V_{OL}$ 變高且更耗電。）}
...
\ans{兩段時間皆約 $2.82$ ns（$t_{pLH}=t_{pHL}$，故 $t_p\approx2.82$ ns）。}
