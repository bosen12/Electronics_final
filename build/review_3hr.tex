...
\begin{formulabox}
$I_D = k\!\left[(V_{GS}-V_t)V_{DS} - \tfrac{1}{2}V_{DS}^2\right]$（三極管區）\qquad
$I_D = \tfrac{1}{2}k(V_{GS}-V_t)^2$（飽和區）
\end{formulabox}

\begin{tipbox}[解題 SOP]
$V_i=V_{DD}$ 時 FET 導通在三極管區，列 $I_D=k[(V_{GS}-V_t)V_o - V_o^2/2] = (V_{DD}-V_o)/R$，解二次方程取\textbf{小根}（$V_o < V_{GS}-V_t$ 才在三極管區）。功率 $P = V_{DD} \cdot I_D$。
\end{tipbox}
