...
\textbf{(1)} $G_m=\sqrt{\dfrac{(1\text{m})(1\text{m})}{2}}=7.07\times10^{-4}$ S，$A_d=-2(7.07\text{m})(5\text{k})=-7.07$。\\
...
%---------------------------------------------------------------
\begin{minipage}{0.60\linewidth}
\prob{圖 P12.10 是 CC 放大器（射極隨耦器），$Q$ 的 B 極偏壓 $V_B=5$ V、$R_L=2$ k$\Omega$、供電 $\pm12$ V。輸出最大擺幅 $\pm4$ V，設計 $R_E$ 以防止 $Q$ 進入截止。}
\end{minipage}\hfill
\begin{minipage}{0.37\linewidth}
...
\[ \frac{4.3-4+12}{R_E}\ge\frac{4}{R_L}\ \Rightarrow\ \frac{12.3}{R_E}\ge\frac{4}{2\text{k}}
   \ \Rightarrow\ R_E\le\frac{12.3\times2\text{k}}{4}=6.15\ \text{k}\Omega. \]
\ans{取 $R_E\le6.15$ k$\Omega$（例如 $R_E=6$ k$\Omega$），即可在 $\pm4$ V 擺幅下避免 $Q$ 截止。}
